...
public class FilesystemDB {
    ...
    public void addDirectoryEntry(DirectoryEntry de) throws SQLException {
        String[] sql = {
    ...
    public void deleteDirectoryEntry(int dirID) throws SQLException {
        PreparedStatement preparedStatement = connection.prepareStatement("DELETE FROM directory_entry WHERE id=?");
        ...
        try (Statement statement = connection.createStatement()) {
            ...
            try (ResultSet resultSet = statement.executeQuery(selectQuery)) {
                while (resultSet.next()) {
                    ...
                    FileEntry fe = new FileEntry(resultSet.getString("name"), resultSet.getInt("size"), resultSet.getInt("id"));
    ...
    public List<DirectoryEntry> getAllDirectories() throws SQLException {
        List<DirectoryEntry> result = new ArrayList<>();
        try (Statement statement = connection.createStatement()) {
            String selectQuery = "SELECT * FROM directory_entry";
            try (ResultSet resultSet = statement.executeQuery(selectQuery)) {
                while (resultSet.next()) {
                    DirectoryEntry de = new DirectoryEntry(resultSet.getString("name"), resultSet.getInt("id"));
